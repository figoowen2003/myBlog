...
- action server是控制长周期任务例如导航的一种常用方法。它是这样的服务器，客户端请求完成某个任务，但该任务需要很长时间。
...
- 生命周期节点（也称为管理节点）是ROS2独有的。他们是这样一类节点，包含了用来启动和关闭ROS2 servers的状态机转换功能。这有助于ROS系统在启动和关闭阶段的决策行为。它还能帮助用户以合理的方式去组织他们的程序，用于商业目的和调试。
- 具体的生命周期过程为：
  - 一个节点启动时，它处于未被配置的状态，只能处理节点的构造行为（不包括任何ROS网络设置和参数读取）。
  ...
  - 为了关闭节点，我们将依次切换到停用、清理、关闭状态，并且以最终状态结束。在以上各个状态下，网络接口将被停用，停止处理消息，释放内存，干净的退出。
...
- 代价地图过滤器 costmap filters
  - 假设，你在一边为地图文件或者任意图片文件做注释，一边根据被注释的地图中的位置去执行特定操作。这个注释或标记可以是防止进入区域以避免规划路径到其中，也可以是让一些像素属于标记区域的最大速度。这个被注释的地图被称为“过滤面具地图”。就好像是一个表面上的一副面具，它的大小，位置和规模相比于住地图可以相同，也可以不同。
